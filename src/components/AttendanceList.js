...
const AttendanceList = ({ attendanceData }) => {
  return (
    <div className="container mx-auto px-4 sm:px-0">
      <h2 className="font-bold text-2xl mb-4 m-6">Attendance List</h2>
      ...
      {attendanceData.map((subject) => {
        ...
        return (
          <div key={_id} className="grid grid-cols-2 gap-4">
            <div className="text-center">{subject_name}</div>
            <div className="text-center">{attendancePercentage}%</div>
          </div>
        );
      })}
    </div>
